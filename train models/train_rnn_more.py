...
def load_series_csv(path: str) -> np.ndarray:
    try:
        ...
        if arr.size == 0: raise ValueError("empty/non-numeric CSV")
        return arr.astype("float32")
    except Exception as e:
        raise RuntimeError(f"read CSV failed: {e}")

# ---------- dataset ----------
class SeqDataset(Dataset):
    """
    окна длины L из нормализованного ряда X(TxD):
...
class LSTMNet(nn.Module):
    def __init__(self, in_dim=1, hid=128, n_layer=2, dropout=0.1):
        super().__init__()
        ...
        self.head = nn.Linear(hid, in_dim)
...
def dump_hidden_states(model, dval: Dataset, device, epoch: int, outdir: str, max_batches: int = 4):
    """
    Снимаем скрытые состояния для первых max_batches вал-окон (batch=1).
    Если max_batches <= 0/None — пишем все окна.
    Сохраняем states_epoch_XXX.npz с H_seq, Y_true, Y_pred.
    """
    model.eval()
...
def dump_hidden_states_dense(model, val_raw: np.ndarray, mean: np.ndarray, std: np.ndarray,
    ...
    """
    Непрерывный прогон по всей валидации (без ресета). Даёт длинный трек скрытых состояний.
    Записывается как дополнительная последовательность в H_seq.
    """
    model.eval()
    X = ((val_raw - mean) / (std + 1e-8)).astype("float32")      # [T,D]
    x = torch.from_numpy(X)[None, ...].to(device)                 # [1,T,D]
    out = model.lstm_outputs(x).squeeze(0).cpu().numpy()          # [T,H]
...
def train(args):
    ...
    model = LSTMNet(in_dim=raw.shape[1], hid=args.hid, n_layer=args.n_layer, dropout=args.dropout).to(device)
