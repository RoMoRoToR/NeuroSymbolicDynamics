...
def train(args):
    ...
    raw = load_series(args.data, args.data_kind, args.resample)
    ...
    model = BiLSTMNextStep(input_dim=raw.shape[1], hidden=args.hidden, num_layers=args.layers, dropout=args.dropout).to(device)
...
def parse_args():
    ...
    p.add_argument("--resample", type=str, default="1H")
    p.add_argument("--outdir", type=str, default="runs/bitcoin/RNN_BiLSTM")
