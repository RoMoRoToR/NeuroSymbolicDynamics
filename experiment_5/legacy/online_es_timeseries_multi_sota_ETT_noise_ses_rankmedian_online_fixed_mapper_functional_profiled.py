...
def parse_args():
    ...
    ap.add_argument("--profile_runtime", action="store_true", help="Record timing (epoch time and ES decision time) into the output CSV")
...
def run_one(csv_path: str, model_name: str, seed: int, noise_sigma: float, args) -> Dict[str, float]:
    ...
    for e in range(1, int(args.epochs) + 1):
        ...
        if str(device).startswith("cuda"):
            torch.cuda.synchronize()
        epoch_times.append(float(time.perf_counter() - t_epoch0))
    ...
    pat_time = float(time.perf_counter() - t0_pat)
    ...
    slope_time = float(time.perf_counter() - t0_slope)
    t0_svcca = time.perf_counter()
    ...
    svcca_time = float(time.perf_counter() - t0_svcca)
    t0_cdsc = time.perf_counter()
    ...
    cdsc_time = float(time.perf_counter() - t0_cdsc)

    # SES (solo v2)
    t0_ses = time.perf_counter()
    ...
    ses_time = float(time.perf_counter() - t0_ses)
    ...
    row: Dict[str, float] = {
        ...
        "ses_repr": str(args.ses_repr),
        "mapper_bins": float(args.mapper_bins),
        "mapper_overlap": float(args.mapper_overlap),
        "mapper_local_k": float(args.mapper_local_k),
        "mapper_merge_eps": float(args.mapper_merge_eps),
        "mapper_max_nodes": float(args.mapper_max_nodes),
    }
    ...
    if bool(getattr(args, "profile_runtime", False)):
        et = np.asarray(epoch_times, dtype=np.float64)
        if et.size > 0:
            et_med = float(np.median(et))
            et_q1 = float(np.percentile(et, 25))
            et_q3 = float(np.percentile(et, 75))
            et_iqr = float(et_q3 - et_q1)
            train_wall = float(np.sum(et))
        else:
            et_med, et_iqr, train_wall = float("nan"), float("nan"), float("nan")
        row["epoch_time_median_sec"] = et_med
        row["epoch_time_iqr_sec"] = et_iqr
        row["train_wall_time_sec"] = train_wall
        row["ses_decision_time_sec"] = float(ses_time) if "ses_time" in locals() else float("nan")
        row["pat_decision_time_sec"] = float(pat_time) if "pat_time" in locals() else float("nan")
        row["slope_decision_time_sec"] = float(slope_time) if "slope_time" in locals() else float("nan")
        row["svcca_decision_time_sec"] = float(svcca_time) if "svcca_time" in locals() else float("nan")
        row["cdsc_decision_time_sec"] = float(cdsc_time) if "cdsc_time" in locals() else float("nan")
        # estimated total wall-clock saved if we had stopped early (based on median epoch time)
        row["ses_est_wall_saved_sec"] = float(et_med) * float(row.get("ses_saved_epochs", 0.0)) if np.isfinite(et_med) else float("nan")
        row["pat_est_wall_saved_sec"] = float(et_med) * float(row.get("pat_saved_epochs", 0.0)) if np.isfinite(et_med) else float("nan")
        row["slope_est_wall_saved_sec"] = float(et_med) * float(row.get("slope_saved_epochs", 0.0)) if np.isfinite(et_med) else float("nan")
        row["svcca_est_wall_saved_sec"] = float(et_med) * float(row.get("svcca_saved_epochs", 0.0)) if np.isfinite(et_med) else float("nan")
        row["cdsc_est_wall_saved_sec"] = float(et_med) * float(row.get("cdsc_saved_epochs", 0.0)) if np.isfinite(et_med) else float("nan")
    return row
