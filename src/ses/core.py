...
class ExperimentConfig:
    ...
    dataset_overrides: Dict[str, Dict[str, object]] = field(default_factory=dict)
...
def resolve_dataset_paths(data_root: Path, datasets: Sequence[str]) -> List[Path]:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            f"Could not resolve dataset(s) under {data_root}: {joined}. "
            "See data/README.md for the expected filenames and preparation steps."
        )
    return resolved
...
def config_for_dataset(cfg: ExperimentConfig, dataset_name: str, csv_path: Optional[Path] = None) -> ExperimentConfig:
    override: Dict[str, object] = {}
    candidate_keys = [str(dataset_name), Path(dataset_name).name]
    if csv_path is not None:
        candidate_keys.extend([csv_path.stem, csv_path.name, str(csv_path)])
        try:
            candidate_keys.append(str(csv_path.resolve().relative_to(Path(cfg.data_root).resolve())))
        except ValueError:
            pass
    seen = set()
    ...
    for key in ordered_keys:
        payload = cfg.dataset_overrides.get(key)
        if payload:
            override.update(payload)
    if not override:
        return cfg
    valid = {item.name for item in fields(ExperimentConfig)}
    invalid = sorted(set(override) - valid)
    if invalid:
        raise ValueError(
            f"Invalid dataset override keys for '{dataset_name}': {', '.join(invalid)}. "
            f"Allowed keys are ExperimentConfig fields."
        )
    return replace(cfg, **override)


def load_dataset_frame(csv_path: Path, cfg: ExperimentConfig) -> pd.DataFrame:
    if not csv_path.exists():
        raise FileNotFoundError(f"Dataset file not found: {csv_path}")
    if csv_path.stat().st_size <= 0:
        raise ValueError(
            f"Dataset file is empty: {csv_path}. "
            "Replace placeholder files with real data or generate the synthetic dataset before running experiments."
        )
    ...
    required = int(cfg.seq_len + cfg.pred_horizon)
    train_len = int(tr_sl.stop - tr_sl.start)
    val_len = int(va_sl.stop - va_sl.start)
    if train_len < required or val_len < required:
        max_seq_len = max(1, min(train_len, val_len) - int(cfg.pred_horizon))
        raise ValueError(
            f"{csv_path.stem} is too short for seq_len={cfg.seq_len} and pred_horizon={cfg.pred_horizon} "
            f"under train/val split {cfg.train_frac:.2f}/{cfg.val_frac:.2f}. "
            f"Available rows after splitting: train={train_len}, val={val_len}; "
            f"use seq_len <= {max_seq_len} or set dataset_overrides."
        )
    return df


def preflight_experiment_inputs(cfg: ExperimentConfig) -> List[Tuple[Path, ExperimentConfig, int]]:
    checked: List[Tuple[Path, ExperimentConfig, int]] = []
    issues: List[str] = []
    for csv_path in resolve_dataset_paths(Path(cfg.data_root), cfg.datasets):
        dataset_cfg = config_for_dataset(cfg, csv_path.stem, csv_path)
        try:
            df = load_dataset_frame(csv_path, dataset_cfg)
        except Exception as exc:
            issues.append(f"- {csv_path.stem}: {exc}")
            continue
        checked.append((csv_path, dataset_cfg, int(len(df))))
    if issues:
        raise RuntimeError(
            "Experiment input validation failed:\n"
            + "\n".join(issues)
            + "\nSee data/README.md for data acquisition notes and configs/*.json for dataset-specific overrides."
        )
    return checked
...
def run_one(csv_path: Path, model_name: str, seed: int, noise_sigma: float, cfg: ExperimentConfig) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    df = load_dataset_frame(csv_path, cfg)
    ...
    train_batch_size = min(int(cfg.batch_size), len(ds_tr))
    val_batch_size = min(int(cfg.batch_size), len(ds_va))
    if train_batch_size <= 0 or val_batch_size <= 0:
        raise ValueError(
            f"{csv_path.stem} produced an empty dataset after windowing; "
            "reduce seq_len or adjust split settings."
        )
    generator = torch.Generator()
    generator.manual_seed(seed)
    dl_tr = DataLoader(ds_tr, batch_size=train_batch_size, shuffle=True, drop_last=False, generator=generator)
    dl_va = DataLoader(ds_va, batch_size=val_batch_size, shuffle=False, drop_last=False)
    ...
    row: Dict[str, float] = {
        ...
        "seq_len": float(cfg.seq_len),
        "batch_size": float(train_batch_size),
        "pred_horizon": float(cfg.pred_horizon),
        ...
        for model_name in cfg.models:
            for sigma in sigmas:
                for i in range(int(cfg.n_runs)):
                    ...
                    all_rows.append(row)
                    if dataset_cfg.save_epoch_metrics:
